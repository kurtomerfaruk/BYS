 ...
 * @version 1.0.0
 * @since 31.07.2025 11:28
 */
@Named
@ViewScoped
@Slf4j
public class HfKasaController extends AbstractController<HfKasa> {
    ...
    public List<SelectItem> getFilterOptions(EnumSyFiltreAnahtari key) {
        switch (key) {
            case HFTAHSILAT_TURU -> {
                return filterOptionService.getHfTahsilatTurs();
            }
            default -> {
    ...
    @Override
    public HfKasa prepareCreate(ActionEvent event) {
        HfKasa newItem;
        try {
            newItem = HfKasa.class.getDeclaredConstructor().newInstance();
            newItem.setHfHafriyatIs(new HfHafriyatIs());
            newItem.setHfFirma(new HfFirma());
            newItem.setTahsilTarihi(LocalDate.now());
    ...
    public void secilenFirma(SelectEvent<HfFirma> event) {
        HfFirma firma = event.getObject();
        this.getSelected().setHfFirma(firma);
    }
